...
name: fable-harness
description: |
  Run an autonomous Opus-Generator + Fable-Verifier harness against a sprint contract with hard ≥98% spec gate,
  file-based handoff, cron auto-unstick, Monitor on status.txt, read-only approved contracts, session-scoped Stop gate,
  optional Codex red-team adversary (prompted to BREAK the build), and CronDelete-self on PASS. Use when the user says
  "fable harness", "fable-harness", "opus builder fable verifier", "opus thinking-high generator", "sprint contract",
  "spec gate", "hard spec gate", "TDD-driven harness", "Generator + Verifier", "auto-unstick", "go end to end don't ask",
  "harness this", or "multi-task autonomous build" AND wants the Opus-thinking-high builder / Fable verifier split (use
  the base `harness-contract` skill instead when the user wants the default Sonnet builder / Opus verifier). Triggers
  strongly when the request bundles ≥3 deliverables, wants measurable must_pass criteria, no further questions, a hostile
  Codex red-team gate, and self-termination when the contract is met.
compatibility: Requires Claude Code/Cowork-style tools for background agents, cron/monitor, and project Stop hooks. The Generator runs on Opus 4.8 with high thinking; the Verifier runs on Fable 5. Optional --codex requires a configured Codex MCP server available to Claude.
---
...
## Model assignments (the defining difference)

| Role | Model | How to spawn |
|---|---|---|
| Generator (builder) | **Opus 4.8, thinking high** | `Agent(subagent_type: general-purpose, model: "opus", run_in_background: true)` and the prompt instructs it to use maximum/high thinking. If your harness supports a per-agent reasoning-effort knob, set it to **high**. |
| Verifier (grader) | **Fable 5** | `Agent(subagent_type: general-purpose, model: "fable")`. Spawned event-driven on `ready_for_evaluation`. |
| Codex red-team (optional) | Codex MCP | Only with `--codex`. Mandate: break the build. |

Model IDs for reference: Opus 4.8 = `claude-opus-4-8`, Fable 5 = `claude-fable-5`. When spawning via the `Agent` tool, the short aliases `opus` and `fable` are what you pass to `model:`.

## Mode flags

- `--codex` (alias `--codex-verify`): add a third **red-team** gate using Codex MCP before terminal completion. Unlike the base skill's neutral verifier, this Codex agent is told its job is to **make the spec gate fail** — to find any hole, any way a deliverable doesn't actually work, any fabricated evidence. The Stop hook does not call MCP itself; it blocks stop until `{SPRINT_DIR}/codex-adversarial-report.md` exists and contains `ADVERSARIAL PASS` (the verdict Codex is only allowed to write when it tried to break the build and genuinely could not).
...
For `--codex` (the red-team gate), always include:

- `codex-adversarial-report.md` placeholder or packet
- `AWAITING_CODEX_VERIFY` state in the cron/status plan
- `ADVERSARIAL PASS` as the exact required terminal phrase (Codex's "I could not break it" verdict)
...
The Generator (**Opus 4.8, thinking high** — deliberate iteration) is spawned in background at launch. The Verifier (**Fable 5**, fast independent grading) is **NOT** spawned at launch — it is spawned **only when `status.txt` flips to `ready_for_evaluation`**, by the Cron (branch d) or the orchestrator reacting to the Monitor transition. When spawned, the Verifier grades **immediately** (status is already `ready_for_evaluation`) and exits — no poll loop.
...
- For Generator: TDD discipline rules + "use high/maximum thinking — reason hard before each edit; you are running on Opus precisely so the implementation reasoning is thorough"
...
### 5. Optional Codex red-team adversary (`--codex`)

When the user requests `--codex` (alias `--codex-verify`), add a third gate — but in this skill it is a **hostile red team, not a neutral second opinion**. Codex is told its job is to **make the spec gate fail**: actively hunt for any deliverable that doesn't truly work, any fabricated TDD evidence, any criterion that passes its shell check but fails its intent, any integration that's green on a static grep but broken at runtime. The build only survives if Codex tries hard to break it and genuinely cannot.

1. Copy `assets/templates/codex-adversarial-prompt.md` into the sprint plan with paths substituted. (This prompt is rewritten for fable-harness to give Codex the break-the-build mandate.)
2. After the Fable Verifier writes `PASS X/M`, call the Codex MCP tool with that prompt.
...
5. Only then restore `status.txt` to `PASS X/M` so Cron can CronDelete and the Stop hook can approve. If the report says `ADVERSARIAL FAIL`, the Cron bounces the findings back to the Generator (status → `implementing`) — a hostile-Codex failure is treated exactly like a Verifier FAIL.
...
10. Spawn Generator (model: opus, run_in_background: true, thinking high) with the prompt from assets/templates/generator-prompt.md, inject paths
11. Do NOT spawn the Verifier yet. It is spawned event-driven (model: fable) when status flips to `ready_for_evaluation` (by the Cron branch d, or by the orchestrator reacting to the Monitor transition). It grades immediately — no poll loop. See moving-part #2.
12. If --codex: prepare assets/templates/codex-adversarial-prompt.md (break-the-build mandate) for cron use after normal PASS
...
| `assets/templates/generator-prompt.md` | Opus (thinking-high) Generator spawn body (with TDD discipline) |
| `assets/templates/verifier-prompt.md` | Fable Verifier spawn body (no leniency, exact commands) |
| `assets/templates/codex-adversarial-prompt.md` | Optional Codex MCP red-team prompt (mandate: break the build) for `--codex` |
