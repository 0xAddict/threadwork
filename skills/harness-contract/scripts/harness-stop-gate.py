...
if codex_verify:
    if not codex_report_path.exists():
        block("harness stop gate: --codex-verify enabled but codex-adversarial-report.md is missing")
    codex_report = codex_report_path.read_text(errors="replace")
    if "ADVERSARIAL PASS" not in codex_report:
        block("harness stop gate: Codex adversarial verifier has not written ADVERSARIAL PASS")
