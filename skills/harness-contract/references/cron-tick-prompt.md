...
3. Branch on CUR:
   ...
   b) CUR matches "PASS X/M" with X < {THRESHOLD_N}, OR CUR starts with "FAIL":
      ...
      - Otherwise (artifact defect): TaskList to check if {GENERATOR_AGENT_NAME}
        ...
        model sonnet, run_in_background true) with a "resume sprint at iter
        T+1" prompt that reads the contract + verifier-report + fixes only the
        FAILing criteria.
   ...
   g) CUR == "AWAITING_CODEX_VERIFY":
      ...
      - If codex-adversarial-report.md contains "ADVERSARIAL FAIL" (the red team
        ...
        ii. SendMessage {GENERATOR_AGENT_NAME} (re-spawn if dead) with the
            codex-adversarial-report.md path and the specific defect(s) Codex
            found — those are now the criteria to fix.
        iii. Telegram alert with the report path: the red-team Codex gate failed
