#!/usr/bin/env bash
# traceability-p7.sh — P6 Stage 8 traceability / ATOM-coverage gate (G6).
#
...
#
# Usage: bash ./scripts/traceability-p7.sh   (run from the P6 worktree)

set -uo pipefail

REPO_DIR="$(cd "$(dirname "$0")/.." && pwd)"
SPEC="/Users/coachstokes/.claude/state/p4-p8-fanout/specs/P7-spec.md"
TESTS_DIR="$REPO_DIR/tests"

overall_exit=0

if [ ! -f "$SPEC" ]; then
  echo "[traceability-p7] FATAL: spec not found at $SPEC" >&2
  exit 1
fi
if [ ! -d "$TESTS_DIR" ]; then
  echo "[traceability-p7] FATAL: tests/ dir not found at $TESTS_DIR" >&2
  exit 1
fi

echo "=================================================================="
echo "[traceability-p7] CHECK 1 — M-001..M-018 manifest + REQ-reference"
echo "=================================================================="
...
if [ -z "$manifest_section" ]; then
  echo "[traceability-p7] FATAL: could not locate '## Traceability manifest' section" >&2
  exit 1
fi
if [ -z "$epics_section" ]; then
  echo "[traceability-p7] FATAL: could not locate '## EPICS' section" >&2
  exit 1
fi
...
if [ "$manifest_orphans" -gt 0 ]; then
  echo "[traceability-p7] CHECK 1 FAILED — $manifest_orphans orphan M-### id(s)"
  overall_exit=1
else
  echo "[traceability-p7] CHECK 1 OK — all M-001..M-018 present in manifest and referenced by >=1 REQ, zero orphans"
fi

echo ""
echo "=================================================================="
echo "[traceability-p7] CHECK 2 — Done-gate P1/P2/P3 atomics -> test coverage"
echo "=================================================================="

donegate_section="$(awk '/^## Done-gate/{p=1; next} /^## /{if (p) exit} p' "$SPEC")"
if [ -z "$donegate_section" ]; then
  echo "[traceability-p7] FATAL: could not locate '## Done-gate' section" >&2
  exit 1
fi
...
if [ -n "$p1_stated" ] && [ "$p1_count" != "$p1_stated" ]; then
  echo "[traceability-p7] FATAL: parser self-check failed — spec states $p1_stated P1 atomics but parsed $p1_count" >&2
  exit 1
fi
if [ -n "$p2_stated" ] && [ "$p2_count" != "$p2_stated" ]; then
  echo "[traceability-p7] FATAL: parser self-check failed — spec states $p2_stated P2 atomics but parsed $p2_count" >&2
  exit 1
fi
if [ -n "$p3_stated" ] && [ "$p3_count" != "$p3_stated" ]; then
  echo "[traceability-p7] FATAL: parser self-check failed — spec states $p3_stated P3 atomics but parsed $p3_count" >&2
  exit 1
fi
if [ -z "$p1_ids" ] || [ -z "$p2_ids" ] || [ -z "$p3_ids" ]; then
  echo "[traceability-p7] FATAL: failed to parse one or more of the P1/P2/P3 atomic lists" >&2
  exit 1
fi
...
# Scope the ATM->test coverage check to P7-OWNED test files only. P6 and P7
# share ATM-0NN numbering (both start at ATM-001), so a repo-wide grep over
# tests/ would let a P6 test file referencing e.g. "ATM-005" spuriously satisfy
# P7's ATM-005 coverage. Every P7 atomic's spec-declared verifier lives in a
# cross-family-critique*.test.ts file, so restrict the grep to those.
P7_TEST_FILES="$(find "$TESTS_DIR" -type f -name 'cross-family-critique*' 2>/dev/null)"
if [ -z "$P7_TEST_FILES" ]; then
  echo "[traceability-p7] NOTE: no P7-owned test files (cross-family-critique*) found under $TESTS_DIR yet — every ATM will read MISSING until the P7 suites land." >&2
fi
...
check_group() {
  ...
  for n in $ids; do
    atm="ATM-$n"
    found="MISSING"
    if [ -n "$P7_TEST_FILES" ] && printf '%s\n' "$P7_TEST_FILES" | xargs grep -lq -- "$atm" 2>/dev/null; then found="found"; fi
    printf "%-9s %-4s %-9s\n" "$atm" "$pri" "$found"
...
if [ "$missing_blocking" -gt 0 ]; then
  echo "[traceability-p7] CHECK 2 FAILED — $missing_blocking P1/P2 atomic(s) have NO test reference under tests/"
  overall_exit=1
else
  echo "[traceability-p7] CHECK 2 OK — every P1/P2 atomic is referenced by >=1 file under tests/ (P3 misses non-blocking)"
fi

echo ""
echo "=================================================================="
echo "[traceability-p7] CHECK 3 — REQ-001..020 rolls up to exactly ONE EPIC-01..07"
echo "=================================================================="
...
for e in 1 2 3 4 5 6 7; do
  var="epic_body_$e"
  if [ -z "${!var}" ]; then
    echo "[traceability-p7] FATAL: could not locate body for EPIC-0$e" >&2
    exit 1
  fi
done
...
if [ "$req_orphans" -gt 0 ]; then
  echo "[traceability-p7] CHECK 3 FAILED — $req_orphans REQ id(s) roll up to zero or more-than-one EPIC"
  overall_exit=1
else
  echo "[traceability-p7] CHECK 3 OK — all REQ-001..020 roll up to exactly one EPIC-01..07, zero orphans"
fi

echo ""
echo "=================================================================="
echo "[traceability-p7] CHECK 4 — ATM-001..030 each carry a valid trailing M-### cell"
echo "=================================================================="
...
if [ "$atm_bad" -gt 0 ]; then
  echo "[traceability-p7] CHECK 4 FAILED — $atm_bad ATM row(s) missing a valid trailing M-### cell"
  overall_exit=1
else
  echo "[traceability-p7] CHECK 4 OK — every ATM-001..030 carries a valid trailing M-### cell (M-001..M-018)"
fi

echo ""
echo "=================================================================="
if [ "$overall_exit" -eq 0 ]; then
  echo "[traceability-p7] ALL CHECKS PASSED — exit 0"
else
  echo "[traceability-p7] ONE OR MORE CHECKS FAILED — exit 1"
fi
echo "=================================================================="
exit "$overall_exit"
