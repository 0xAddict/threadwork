#!/usr/bin/env bats
# C3.2 — Tier B stops pinging when Tier A heartbeat stale → external alarm fires per healthchecks.io grace
...
@test "C3.2: pinger sends ping when sentinel heartbeat is fresh" {
...
@test "C3.2: pinger does NOT ping when sentinel heartbeat is stale" {
  python3 -c "
import os, time
...
"
  ...
  # Either log is absent or doesn't contain curl-called (sentinel stale → no ping)
  if [ "$status" -eq 0 ]; then
...
@test "C3.2: missing sentinel heartbeat stops pinging" {
