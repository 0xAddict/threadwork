# Sprint 3 Approved Contract

**status: APPROVED**

## Verifier Review Notes

All 8 criteria are objective, measurable, and verifiable via test execution and code inspection. No modifications required. Minor precision notes are documented below for the Generator's awareness but do not block approval.

---

## Accepted Acceptance Criteria (verbatim from proposal)
...
4. **Decision monitoring metrics in cycle summary**: The watchdog `run()` loop's cycle summary log line includes `decisions_expired=`, `decisions_nudged=`, `decisions_ready=`, and `idle_nudges=`. Verify the log line is emitted correctly by running a cycle with at least one of each event type and checking that the log output contains all four counter names with non-zero values.

5. **Edge case: agent opens decision and is the only expected responder**: When an agent opens a decision AND is the only worker agent (simulated by having all other agents with active tasks), the opener should still be nudged for their own decision after 10 minutes. Test: create decision opened by 'steve', backdate 15 min, verify steve is included in the nudge target list.

6. **Decision position nudge does NOT fire for expired decisions**: When a decision is both old (>10 min) AND has an expired deadline, `monitorDecisions()` must expire it first and then NOT issue a position nudge for it. Test: create decision with expires_at in the past AND created_at 15 min ago, run monitorDecisions, verify decisions_expired=1 AND decisions_nudged=0.
...
8. **Agent-scoped open_decisions count in idle nudge**: Fix the Sprint 2 finding — the idle nudge open_decisions count must exclude decisions where the agent already has a position. Update the query in `monitorIdleAgents()` accordingly. Test: create agent with position already submitted on all open decisions, verify they are NOT nudged for open decisions.

---

## Definition of Done (verbatim from proposal)
...
## Verifier Notes for Evaluation

The following precision notes will be checked during evaluation:

- **Criterion 2**: The contract allows two possible mechanisms for preventing double-nudging (cooldown check OR open_decisions count drops). The Generator must implement and test at least one explicitly. The verifier will test that the outcome (no double nudge) is achieved regardless of mechanism.
- **Criterion 5**: "Included in the nudge target list" must be demonstrated either via a test assertion on the actual nudge call or by verifying the audit log contains a `decision_position_nudge` for steve.
- **Criterion 8**: The fix must be a SQL-level change (a subquery or JOIN against `decision_positions`), not application-level filtering. The verifier will inspect the query in `monitorIdleAgents()`.
